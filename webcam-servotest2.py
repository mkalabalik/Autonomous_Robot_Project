...
##        self.servoGovde = 12
        ...
        self.intServoIleri = 450
        self.intServoYukari = 450
        self.intServoAgiz = 450
##        self.intServoGovde = 450
...
##        self.motor2B = 13
        ...
        self.PWM = GPIO.PWM(self.pwmPin, 500) #pin, frequency
        self.PWM.start(0)
        self.duty = 50
    ...
    def servoReset(self):
        self.intServoYukari = 450
        self.intServoIleri = 550
        self.intServoAgiz = 450
        self.pwm.set_pwm(self.servoYukari, 0, self.intServoYukari)
        self.pwm.set_pwm(self.servoIleri, 0, self.intServoIleri)
        self.pwm.set_pwm(self.servoAgiz, 0, self.intServoAgiz)
##        self.pwm.set_pwm(self.servoGovde, 0, self.intServoGovde)

    def servoOynat(self, kanal, konum, sonKonum):
        if sonKonum-konum > 0 :
            delta = 1
        else:
            delta = -1

        while not konum == sonKonum:
            self.pwm.set_pwm(kanal, 0, konum)
            time.sleep(0.01) 
            konum = konum + delta

        if kanal == self.servoIleri:
            self.intServoIleri = konum
        elif kanal == self.servoYukari:
            self.intServoYukari = konum
        elif kanal == self.servoAgiz:
            self.intServoAgiz = konum
##        elif kanal == self.servoGovde:
##            self.intServoGovde = konum
            
        
    def hizAyari(self):
        print("hız ayarı devredışı")
...
##            print(event)
            ...
            if event.type == pygame.JOYBUTTONDOWN:
                if event.button == 8:
                    ...
                    robot.temizle()
                    pygame.quit()
                    print("sys.exit çalışacak")
                    sys.exit()

                if event.button == 0:
                    robot.servoOynat(robot.servoAgiz, robot.intServoAgiz, robot.intServoAgiz+10)
                if event.button == 1:
                    robot.servoOynat(robot.servoAgiz, robot.intServoAgiz, robot.intServoAgiz-10)
                if event.button == 2:
                    robot.servoOynat(robot.servoYukari, robot.intServoYukari, robot.intServoYukari+10)
                if event.button == 3:
                    robot.servoOynat(robot.servoYukari, robot.intServoYukari, robot.intServoYukari-10)
                if event.button == 4:
                    robot.duty -=1
                    print("--------------")
                    print(robot.duty)
                    robot.PWM.ChangeDutyCycle(robot.duty)
##                    robot.servoOynat(robot.servoIleri, robot.intServoIleri, robot.intServoIleri-10)
                if event.button == 5:
                    robot.duty +=1
                    print("++++++++++++++++")
                    print(robot.duty)
